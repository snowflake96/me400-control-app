...
public:
    ...
    void setServoSpeed(uint8_t channel,
                      ...
                      const float cw_max_us = 1680.0f,
                      const float ccw_min_us = 1840.0f,
                      const float ccw_max_us = 2500.0f);
...
inline void PCA9685Driver::setESC(uint8_t channel,
  ...
  static constexpr float center_us = 1740.0f;
  static constexpr float span_us   = 500.0f;

  if (std::isnan(throttle)) return;
  // clamp to [0, +SAFETY_LIMIT]
  throttle = std::clamp(throttle, 0.f, SAFETY_LIMIT);
  // linear map around center
  float pulse_us = center_us + throttle * span_us;
